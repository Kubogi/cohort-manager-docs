...
- **Link to a user account** → pick from the User dropdown. The `(taiKhoan)` partial unique index allows multiple rows with no link but at most one row per linked User.
