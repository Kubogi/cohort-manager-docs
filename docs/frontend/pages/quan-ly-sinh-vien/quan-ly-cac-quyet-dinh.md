...
- **`Loại` advanced filter** is **exact-match** against each student's *latest* decision (newest `ngayKiQD`). Students whose newest decision has a different `loaiQD` are excluded. The full decision set is loaded up-front so this latest-per-student join uses the actual newest, not a paginated subset.
