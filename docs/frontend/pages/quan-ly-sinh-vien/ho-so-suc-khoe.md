...
**What it does:**
- Filter by `khoa`/`daiDoi`/`trangThai` (Bình thường / Viện). `trangThai` narrows Tab 1 with the **"students with those records"** semantic: filtering Viện shows every student with ≥1 `trangThai='Viện'` record; filtering Bình thường shows every student with ≥1 `trangThai='Bình thường'` record. A mixed-history student (≥1 BT + ≥1 Viện record) appears under BOTH filters — they have records of both kinds — even though the Trạng thái column on their row reads "Viện" (their current state). Students with no records appear under NEITHER filter. The hook derives this from the trangThai-invariant `statsRecords` set (perPage=10000) — same dataset Tab 2 and Tab 3 read — so the result is consistent regardless of where the user toggled the filter.
- **Pagination semantics under any restrictive trangThai filter** (Viện-only OR Bình thường-only OR none): the table slices the FULL derived list client-side using `studentsPagination`. The footer total reads the derived count (e.g. "1–50 of 44"), not the server's student total. Paging the slice does NOT trigger a student refetch — the data is already complete in `statsRecords` + `studentById`. Clearing back to "both" reverts to server-paginated student fetching with the full student total in the footer.
...
- **Trạng thái column resolves from the row's own student**, not by re-scanning `data.students` via `maSV`. The earlier implementation called `getHealthStatus(student.maSV)` which did `data.students.find(s => s.maSV === maSV)` — fine for in-page students, but off-page students surfaced into `displayedStudents` via the supplemented `studentById` aren't in `data.students`. The lookup returned `undefined` and the column defaulted to "Bình thường" — visibly contradicting the action-column button (which used `student.id` directly and correctly showed the row as hospitalized). The column now derives from the same `activeRecord = healthRecords.find(r => r.sinhVien === student.id && r.trangThai === 'Viện')` lookup that drives the action buttons, so the two views agree for every row regardless of pagination.
...
**What it does:**
- Per-record detail view with a richer column set than Tab 1.
- Inline editing of any row.

**What it shows:**
- One row per health record. The list is **invariant under the Tab 1 trangThai filter**: Tab 2 always renders every record in the active base + advanced scope (Tab 1's trangThai toggle is a Tab-1-only narrowing). Pre-fix, picking Viện on Tab 1 silently dropped every Bình thường record from Tab 2 too — the user had no UI indication of why their record count had collapsed, since Tab 2 doesn't expose its own trangThai control.
- More columns than Tab 1 — includes `khung`, `nguoiDiKem`, `chuanDoanBenhVien`, `sinhVienDiCung`.

**Pagination is client-side** over the loaded `statsRecords` set (perPage=10000 main fetch covers every record in scope). `goToRecordsPage` / `changeRecordsPageSize` re-slice without a network call. The footer total reads `statsRecords.length` so it stays stable when the user toggles trangThai on Tab 1.
...
- **Số ca đi viện** is the count of *records* per đại đội, not the count of students with ≥1 record — a student with three separate health records contributes three to the column. The count is **invariant under the Tab 1 trangThai filter**: Báo cáo always reports total hospital visits (every admission, past or present), regardless of whether the user has set "Viện only" or "Bình thường only" on Tab 1. The hook achieves this by sourcing stats from a separate `statsRecords` dataset fetched without the `trangThai` param; the visible `khoa` / `daiDoi` / `donViLienKet` / `hoTen` / `maSV` / `lop` / `ngaySinh` filters still apply. Without this decoupling, picking Viện on Tab 1 silently collapsed the column to the currently-active subset (e.g. 221 → 41) with no UI indication of *why* the number had dropped, since Báo cáo has no trangThai control of its own.
- **Catalog cap matters for the displayed total.** The rollup builds one row per đại đội in `unitLookups.daiDoiList`. Records assigned to a đại đội missing from that list have their counts computed (in the internal map) but never rendered, so the displayed total under-reports. The frontend pagination cap for `don-vi/dai-doi` is set high enough (10000) to cover any realistic deployment. A lower cap was the production reproduction of the "240 records collapsing to ~57" symptom: K47 → K181 with dozens of đại đội per khóa easily exceeds the legacy 200-row cap.
...
- **Save resolves the student via the row context, not by re-scanning the paginated `students` page.** When a row from a filtered view (Thống kê, or any Hồ sơ filter that pulled records past the student-page slice) is edited, the linked student often lives off the current paginated `students` list. Earlier versions resolved the student in `proceedSave` via `students.find(...)` and fired "Không tìm thấy sinh viên với mã đã nhập" on save — both for plain saves and for the "Xác nhận xuất viện" discharge confirm. The save path now prefers the `addForStudent` prop the calling row passed in (same source the field uses for prefill), so the off-page case writes correctly. Hand-typed Mã SV in the create flow still resolves through `students` as before.
