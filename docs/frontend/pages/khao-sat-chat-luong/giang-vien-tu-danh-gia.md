...
- **No data captured here.** The form responses go wherever the URL points (typically Google Forms → a Sheet). This page is purely a redirector.
- **Result file archive moved to [Kết quả khảo sát](./ket-qua-khao-sat.md).** Earlier versions hosted an admin-only upload slot here; admins now upload + archive results on the Kết quả khảo sát page (gvTuDanhGia tab), where processing also runs.
