...
- **`trangThai ∈ { 'Học ghép', 'Học lẻ' }`** — per-subject `tbMon` is still computed normally and grade entry remains editable, but the **cross-subject `diemTB` summary is suppressed** to `—` everywhere it appears (Tổng kết cuối khóa table, xếp loại tally). The two mechanisms compose independently: a student that is both in a suppress-rollup status AND has `monMienHoc` exemptions will see per-subject dashes for exempt subjects AND a cross-subject `—` from the status. The suppress-rollup set lives at `TRANG_THAI_SUPPRESS_ROLLUP` (backend `models/SinhVien.js`, frontend `data/constants.ts`).
